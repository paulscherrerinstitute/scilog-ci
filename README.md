...
cd scilog-ci/helm_configs/frontend/development/
...
Once the merge request CI is successful, the user can merge on the `main` branch. If the configuration changed was a `qa` one, namely if the file changed is in `helm_configs/{component}/qa` or there is a change in the component submodule, then the component is deployed on the `qaprod` cluster, using the `scilog-qa` namespace, otherwise, no deployment on `qa` takes place.
